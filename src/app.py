...
def create_app(test_config=None):
    ...
    def client_parkings():
        if request.method == "POST":
            ...
            # водитель c client_id может заехать
            data = request.json
            opened = (
                db.session.execute(
                    select(Parking.opened).where(
            ...
            if opened:
                ...
                # узнаем количество свободных парковочных мест
                ...
                # снова узнаем сколько осталось свободных парковочных мест
                count_available_places = (
                    db.session.execute(
                        select(Parking.count_available_places).where(
                            Parking.id == data["parking_id"]
                ...
                # если на стоянке больше мест нет - меняем статус
                # парковки на "закрыто"
            ...
            """выезд с парковки (количество свободных мест увеличивается,
            проставляем время выезда). В теле запроса передать client_id,
            parking_id. При выезде — производить оплату
            ...
            # проверяем, привязана ли у клиента банковская карта к аккаунту,
            # если карта привязана к аккаунту делаем вид, что платёж прошел.
            card = (
                db.session.execute(
